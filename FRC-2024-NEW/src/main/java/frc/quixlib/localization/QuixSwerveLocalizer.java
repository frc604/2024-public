...
public class QuixSwerveLocalizer {
  ...
  public void update(
    ...
    Logger.recordOutput("Localizer/UpdateMs", (endTimestampUs - startTimestampUs) / 1000.0);
  }
  ...
  public void updateWithLatestPoseEstimate() {
    ...
    final NTPoseEstimate estimate = m_networkTable.getLatestPoseEstimate();
    ...
    if (estimate.getID() == m_lastUpdatedID || !estimate.hasVision()) {
      final double endTimestampUs = RobotController.getFPGATime();
      Logger.recordOutput(
          "Localizer/UpdateWithLatestPoseEstimateMs", (endTimestampUs - startTimestampUs) / 1000.0);
