...
public class QuixTalonFXTest implements AutoCloseable {
  ...
  public void setup() {
    ...
    controller_follower =
        new QuixTalonFX(new CANDeviceID(1), controller, true, QuixTalonFX.makeDefaultConfig());
    ...
    controller_inverted =
        new QuixTalonFX(
            new CANDeviceID(2),
            new MechanismRatio(),
            QuixTalonFX.makeDefaultConfig().setInverted(true));
  ...
  public void testSetGetPercentOutput() {
    ...
    controller_follower.updateInputs();
  ...
  public void testSetGetPercentOutputInverted() {
    ...
    Timer.delay(0.1);
    controller_inverted.updateInputs();
  ...
  public void testEncoderSetPosition() {
    ...
    controller.updateInputs();
    controller_inverted.updateInputs();
    controller_with_ratio.updateInputs();
  ...
  public void testSimSensorInverted() {
    ...
    Timer.delay(0.1);
    controller_inverted.updateInputs();
    controller_with_ratio_inverted.updateInputs();
  ...
  public void testPositionControl() {
    ...
    assertEquals(controller.getClosedLoopReference(), 100.0, kTol);
    assertEquals(controller_with_ratio.getClosedLoopReference(), 100.0, kTol);
  ...
  public void testSoftLimits() {
    ...
    final QuixTalonFX motorWithoutLimit =
        new QuixTalonFX(
            new CANDeviceID(6), new MechanismRatio(1.0, 2.0), QuixTalonFX.makeDefaultConfig());

    // Within soft limits. Fwd/rev should both work.
    motorWithLimit.setPercentOutput(0.5);
    motorWithoutLimit.setPercentOutput(0.5);
    Timer.delay(0.1);
    motorWithLimit.updateInputs();
    motorWithoutLimit.updateInputs();
