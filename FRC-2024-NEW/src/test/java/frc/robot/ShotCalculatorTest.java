...
public class ShotCalculatorTest {
  ...
  private double computeStaticShotHeightAtGoalInches(double xPosInches, double elevationDegrees) {
    final double horizontalVel =
        Constants.Launcher.defaultShotVelocity * Math.cos(Math.toRadians(elevationDegrees));
    final double verticalVel =
        Constants.Launcher.defaultShotVelocity * Math.sin(Math.toRadians(elevationDegrees));
    final double t =
  ...
  public void testStationary() {
    ...
    // checkStaticShot(135.0, 28.0); // Distance-based velocity ramping
    // checkStaticShot(171.0, 23.0); // Distance-based velocity ramping
