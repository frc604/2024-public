...
def plan_8(quiet):
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "partial/876fast-8")

    # Plot
    field.plot_traj(robot, traj, "partial/876fast-8.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "partial/876fast-8.gif", save_gif=False)
...
def plan_8s7s(quiet):
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "partial/876fast-8s7s")

    # Plot
    field.plot_traj(robot, traj, "partial/876fast-8s7s.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "partial/876fast-8s7s.gif", save_gif=False)
