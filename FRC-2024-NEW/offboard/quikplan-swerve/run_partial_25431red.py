...
def plan_2s5(quiet):
    ...
    qp.add_waypoint(
        waypoint5a,
        10,
        end_constraints=[XYConstraint(waypoint5a), GoalConstraint()],
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "partial/25431red-2s5")

    # Plot
    field.plot_traj(robot, traj, "partial/25431red-2s5.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "partial/25431red-2s5.gif", save_gif=False)
...
def plan_5s4(quiet):
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "partial/25431red-5s4")

    # Plot
    field.plot_traj(robot, traj, "partial/25431red-5s4.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "partial/25431red-5s4.gif", save_gif=False)
...
def plan_54(quiet):
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "partial/25431red-54")

    # Plot
    field.plot_traj(robot, traj, "partial/25431red-54.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "partial/25431red-54.gif", save_gif=False)


def plan_4s3s1s(quiet):
    # Create the field
    field = Field(obstacles=False)
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "partial/25431red-4s3s1s")

    # Plot
    field.plot_traj(robot, traj, "partial/25431red-4s3s1s.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "partial/25431red-4s3s1s.gif", save_gif=False)
