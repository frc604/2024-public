...
def plan(quiet):
    ...
    inside_stage_waypoint = (5.0, 4.6, np.pi)
    qp.add_waypoint(
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "786")

    # Plot
    field.plot_traj(robot, traj, "786.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "786.gif", save_gif=False)
