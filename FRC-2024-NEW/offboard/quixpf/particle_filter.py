...
ROBOT_TO_CAMERA_LEFT = tf.compose_matrix(
    translate=[in2m(10.37), in2m(13.64), in2m(19.56)],  # x,y,z from origin
    angles=[0.0, np.deg2rad(-22.5), 0.0],
)
ROBOT_TO_CAMERA_RIGHT = tf.compose_matrix(
    translate=[in2m(10.37), in2m(-13.64), in2m(19.56)],
    angles=[0.0, np.deg2rad(-24.5), 0.0],  # Manually tuned extrinsics
)
...
class ParticleFilter:
        ...
        for measurement in measurements:
            ...
            if target is None:
                ...
                likelihood = self.compute_single_measurement_likelihoods(
                    measurement, T[0, 3], T[1, 3], T[2, 3], measurement.targetID
                )
                has_at_least_one_measurement = True
            else:
                # AprilTag Center
                likelihood = self.compute_single_measurement_likelihoods(
                    measurement, target.x, target.y, target.z, measurement.targetID
                )
                has_at_least_one_measurement = True
    ...
    def compute_single_measurement_likelihoods(
        self, measurement, x, y, z, targetID=None
    ):
        ...
        beSigma = 5e-3 if targetID in SPEAKER_TAG_IDS else 1e-2
        elSigma = 5e-3 if targetID in SPEAKER_TAG_IDS else 1e-2
