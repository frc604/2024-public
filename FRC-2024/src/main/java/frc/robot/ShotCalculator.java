...
public class ShotCalculator {
  ...
  public static ShotInfo computeSpeakerShotInfo(
    ...
    final Twist2d dTwist =
        new Twist2d(
            robotVel.getX() * TimedRobot.kDefaultPeriod
                + 0.5 * robotAccel.getX() * TimedRobot.kDefaultPeriod * TimedRobot.kDefaultPeriod,
            robotVel.getY() * TimedRobot.kDefaultPeriod
                + 0.5 * robotAccel.getY() * TimedRobot.kDefaultPeriod * TimedRobot.kDefaultPeriod,
            robotVel.getRotation().getRadians() * TimedRobot.kDefaultPeriod
                + 0.5
    ...
    final double elevationVelocity =
        (nextElevationAndYawInfo.elevation - elevation) / TimedRobot.kDefaultPeriod;
  ...
  private static StaticShotInfo computeElevationAngleAndYaw(
    ...
    final double shotVelocity = Constants.Launcher.shotVelocity;
    ...
    for (int i = 0; i < 10; i++) { // 10 iterations for elevationAngle to converge
      ...
      final double shotHorizontalVel = Constants.Launcher.shotVelocity * Math.cos(elevationAngle);
      ...
      for (int j = 0; j < 10; j++) { // 10 iterations for yaw to converge
        yaw = (lbYaw + ubYaw) * 0.5;
        shotVelVector =
      ...
      final double totalShotVerticalVel =
          Constants.Launcher.shotVelocity * Math.sin(elevationAngle);
  ...
  public static ShotInfo computeFeedShotInfo(
    ...
    final double yawVelocity =
        (MathUtils.placeInScope(nextShotInfo.yaw, yaw) - yaw) / TimedRobot.kDefaultPeriod;
    final double elevation = shotInfo.elevation;
    final double elevationVelocity =
        (nextShotInfo.elevation - elevation) / TimedRobot.kDefaultPeriod;
