...
public class AutoAim extends Command {
  ...
  private final PIDController m_yawController = new PIDController(3.0, 0.0, 0.5);
  ...
  public void execute() {
    ...
    final boolean isBPressed = m_xboxController.getBButton();
    final boolean isDpadSidePressed =
        m_xboxController.getPOV() == 90 || m_xboxController.getPOV() == 270;
    final boolean shootButtonPressed = isBPressed || isDpadSidePressed;
    ...
    final double thetaVel =
        (Math.abs(currentTheta - m_yawState.position) > Math.toRadians(1.0)
    ...
    SmartDashboard.putNumber("AutoAim theta (degrees)", Math.toDegrees(currentTheta));
    SmartDashboard.putNumber("AutoAim theta target (degrees)", Math.toDegrees(shotInfo.yaw));
    ...
    SmartDashboard.putNumber("Shot Velocity (meters per second)", shotInfo.shotVelocity);
    ...
    SmartDashboard.putBoolean("AutoAim: Shot Feasible", shotInfo.feasibleShot);
    SmartDashboard.putBoolean("AutoAim: Yaw OK", yawOK);
    SmartDashboard.putBoolean("AutoAim: Elevation OK", elevationOK);
    SmartDashboard.putBoolean("AutoAim: Launch OK", launchOK);
    SmartDashboard.putBoolean("AutoAim: Elevator OK", elevatorOK);
    SmartDashboard.putNumber("AutoAim: Distance To Goal (ft)", Units.metersToFeet(goalDistance));
  }
