...
def plan(quiet):
    ...
    qp.add_waypoint(
        ...
        intermediate_constraints=[GoalConstraint()],
        end_constraints=[
            XYConstraint(outside_stage_shot_waypoint),
        ],
    )

    outside_stage_shot_waypointb = (4.3, 5.2, np.pi)
    qp.add_waypoint(
        outside_stage_shot_waypointb,
        10,
        intermediate_constraints=[GoalConstraint(), SpeedConstraint(0.5)],
        end_constraints=[
            StoppedXYConstraint(outside_stage_shot_waypointb),
        ],
        end_action=Action(ActionType.ENABLE_SHOOT),
    )
    ...
    # Plan the trajectory
    traj = qp.plan()
    write_to_csv(traj, "867slow")

    # Plot
    field.plot_traj(robot, traj, "867slow.png", save=True, quiet=quiet)

    if not quiet:
        # Animate
        field.anim_traj(robot, traj, "867slow.gif", save_gif=False)
